...
import { useBillingInvoice } from "../Billing/useBilling";
import { MoneyFormatter } from "../../shared/components/Numbers/MoneyFormatter";
import { getDifferenceDays } from "./Util";
export const InvoicePaymentsCard = () => {
    const [page, setPage] = useState(1);
    const [pageSize, setPageSize] = useState(5);
    const { billing, count, pager } = useBillingInvoice(page, pageSize);
        ...
        <div className="bg-white rounded-lg shadow p-4 ">
            <div className="flex items-center mb-4">
                ...
                <h2 className="text-lg font-bold">
                    Facturas por Cobrar</h2>
            </div>
            {billing?.map((bill) => (
                <Link to={bill.typeDocument === "Formula" ? `/formulas/${bill.id}` : `/Sales/${bill.id}`}
